/**
 * Alternative News APIs Integration
 * 
 * Provides fallback content sources when RSS feeds fail
 * Supports multiple news APIs with different coverage areas
 */
...
/**
 * Fetch articles from multiple news APIs as RSS fallback
 */
export async function fetchFromAlternativeAPIs(
  ...
  for (const config of configs) {
    try {
      ...
      // Respect rate limits
      await sleep(60000 / config.rateLimit); // Convert to milliseconds between requests
    } catch (error) {
      results.push({
        source: config.name,
        articles: [],
...
async function fetchFromNewsAPI(
  ...
  try {
    ...
    const kenyaResponse = await fetch(kenyaUrl.toString(), {
      headers: {
    ...
    if (!kenyaResponse.ok) {
      throw new Error(`NewsAPI Kenya: HTTP ${kenyaResponse.status}`);
    }

    const kenyaData = await kenyaResponse.json();
    
    // Process Kenya articles
    for (const article of kenyaData.articles || []) {
    ...
    // Also fetch East Africa news if we need more articles
    if (articles.length < maxArticles / 2) {
      const eastAfricaUrl = new URL(`${config.baseUrl}/everything`);
      eastAfricaUrl.searchParams.set('q', 'Kenya OR "East Africa" OR Nairobi');
      eastAfricaUrl.searchParams.set('language', 'en');
      eastAfricaUrl.searchParams.set('sortBy', 'publishedAt');
      eastAfricaUrl.searchParams.set('pageSize', '50');
      eastAfricaUrl.searchParams.set('apiKey', config.apiKey);

      const eastAfricaResponse = await fetch(eastAfricaUrl.toString());
      
      if (eastAfricaResponse.ok) {
        const eastAfricaData = await eastAfricaResponse.json();
        
        for (const article of eastAfricaData.articles || []) {
          if (article.title && article.url && article.publishedAt) {
            const publishedTime = new Date(article.publishedAt).getTime();
    ...
    return {
      ...
      success: true,
      rateLimitRemaining: parseInt(kenyaResponse.headers.get('X-RateLimit-Remaining') || '0'),
    };
...
/**
 * Reuters API integration (if available)
 */
async function fetchFromReutersAPI(
  ...
  // Reuters doesn't have a public API, but we can implement RSS parsing for their feeds
  try {
    const reutersFeeds = [
  ...
  try {
    ...
    while ((match = itemRegex.exec(xmlText)) !== null) {
      ...
      if (title && link && pubDate) {
        ...
        if (publishedTime > lastTimestamp) {
          // Only include Africa-related news
...
/**
 * Categorize by Guardian section
 */
...
export function createDefaultAPIConfigs(apiKeys: Record<string, string>): NewsAPIConfig[] {
  ...
  if (apiKeys.guardian) {
    configs.push({
      ...
      rateLimit: 12, // 12 requests per second for free tier
      coverage: ['Global', 'Africa'],
    });
  }

  // Reuters and BBC don't require API keys (RSS-based)
  configs.push({
    name: 'Reuters',
    baseUrl: 'https://www.reuters.com',
    apiKey: '',
    rateLimit: 6, // Conservative rate limit for RSS
    coverage: ['Global', 'Africa'],
  });
